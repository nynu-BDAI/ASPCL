...
COCO_DATASET_ROOT=os.getenv("IMG_DATA_DIR","../data/")
...
class NpyDataset(Dataset):
    ...
    def __getitem__(self,index):
        ...
        self.tokenize([str(self.samples[index]['adj_swap_aug_caption'])])[0],
        ...
        self.tokenize([str(self.samples[index]['restructure_add_obj_negative_caption'])])[0], 
        self.tokenize([str(self.samples[index]['add_att_negative_caption'])])[0],
        self.tokenize([str(self.samples[index]['swap_positive_caption'])])[0],
        self.tokenize([str(self.samples[index]['replace_positive_caption'])])[0]
        ])
        if self.data=='coco':
...
class HardNegative_Collate:
    def __call__(self,batch):
        ...
        captions = torch.stack([example[1] for example in batch]) # -> [batch_size, 10, seq_len]
        
        text = captions.view(-1, captions.shape[-1])
