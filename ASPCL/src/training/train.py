...
def train_one_epoch(model, data, loss, epoch, optimizer, scaler, scheduler, args, tb_writer=None):
   ...
   # else:
    ...
    for i, batch in enumerate(dataloader):
        ...
        if args.accum_freq == 1:
            with autocast():
                ...
                if args.extra_da:
                    ...
                    total_loss, thresholds, cmr_loss, imc_loss, analogy_loss, positive_loss = loss(image_features, text_features,valid_caption_mask, logit_scale,thresholds)
        ...
        if is_master(args) and args.save_frequency < 0 and (step > 0) and (step % abs(args.save_frequency)) == 0: 
            checkpoint_dict = {
                "epoch": epoch,
                "step": step,
                "name": args.name,
            ...
            torch.save(
                checkpoint_dict,
                os.path.join(args.checkpoint_path, checkpoint_filename),
            )
        ...
        if is_master(args) and (i_accum % args.log_every_n_steps == 0 or batch_count == num_batches_per_epoch):
            ...
            logging.info(
                ...
                f"CMR_Loss: {cmr_loss.item() * args.cmr_loss_weight if cmr_loss is not None else None} "
                f"IMC_Loss: {imc_loss.item() * args.imc_loss_weight if imc_loss is not None else None} "
                f"Analogy_Loss: {analogy_loss.item() * args.analogy_loss_weight if args.analogy_loss and analogy_loss is not None else None} "
                f"PosMargin_Loss: {positive_loss.item() * args.positive_margin_loss_weight if args.positive_margin_loss and positive_loss is not None else None} "
            )
            ...
            if args.cmr_loss and args.threshold_type=="fixed":
                ...
                threshold_logs = [f"Threshold_{i+1}: {th.item():.3f}" for i, th in enumerate(thresholds.mean(0))]
                logging.info(" ".join(threshold_logs))
            ...
            log_data = {
                ...
                "Analogy_Loss_Weighted": analogy_loss.item() * args.analogy_loss_weight if args.analogy_loss and analogy_loss is not None else None,
                "PosMargin_Loss_Weighted": positive_loss.item() * args.positive_margin_loss_weight if args.positive_margin_loss and positive_loss is not None else None
            }
            ...
            if args.cmr_loss and args.threshold_type=="fixed":
                ...
                for i, th in enumerate(thresholds.mean(0)):
                    log_data[f"threshold_{i+1}"] = th.item()
